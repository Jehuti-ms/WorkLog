...
const CLOUD_CONFIG = {
    ...
    masterKey: '$2a$10$6MzZk7r7vLbe6OoD6R1Sx.X7pJ5u5b5J5n5b5J5n5b5J5n5b5J5n5b5', // Free public key
    headers: {
        'Content-Type': 'application/json',
        'X-Master-Key': '$2a$10$6MzZk7r7vLbe6OoD6R1Sx.X7pJ5u5b5J5n5b5J5n5b5J5n5b5J5n5b5'
    }
};
...
function initializeCloudSync() {
    ...
    if (typeof Auth !== 'undefined' && Auth.isAuthenticated()) {
        const userId = Auth.getCurrentUserId();
        const userBinId = localStorage.getItem(`worklog_bin_${userId}`);
        
        if (userBinId) {
            cloudSync.binId = userBinId;
            ...
            // Create cloud storage for new user
            setTimeout(() => createCloudBin(), 3000);
        }
...
// Create a new cloud bin for user
async function createCloudBin() {
    ...
    try {
        ...
        cloudSync.binId = result.metadata.id;
        cloudSync.lastSync = new Date();
        cloudSync.enabled = true;
        
        // Save bin ID for this user
        localStorage.setItem(`worklog_bin_${userId}`, cloudSync.binId);
        localStorage.setItem('worklog_last_sync', cloudSync.lastSync.toISOString());
        
        console.log('Cloud bin created:', cloudSync.binId);
        showNotification('✅ Cloud sync enabled! Your data is now backed up.', 'success');
        
    } catch (error) {
        console.error('Error creating cloud bin:', error);
        showNotification('⚠️ Cloud sync setup failed. Working offline.', 'warning');
...
// Real cloud sync function
async function manualSyncToCloud() {
    if (!cloudSync.enabled || !cloudSync.binId || cloudSync.syncing) return;
    ...
    try {
        ...
        const response = await fetch(`${CLOUD_CONFIG.baseUrl}/${cloudSync.binId}`, {
            method: 'PUT',
            headers: CLOUD_CONFIG.headers,
            body: JSON.stringify(data)
        });
        ...
        cloudSync.lastSync = new Date();
        cloudSync.lastLocalChange = null; // Reset local changes flag
        localStorage.setItem('worklog_last_sync', cloudSync.lastSync.toISOString());
...
async function loadFromCloud() {
    ...
    try {
        const response = await fetch(`${CLOUD_CONFIG.baseUrl}/${cloudSync.binId}/latest`, {
            headers: {
        ...
        const cloudLastUpdated = new Date(cloudData.metadata?.lastUpdated || 0);
        const localLastUpdated = new Date(localStorage.getItem('worklog_last_sync') || 0);
        
        if (cloudLastUpdated > localLastUpdated) {
            console.log('Newer data found in cloud, merging...');
...
async function mergeCloudData(cloudData) {
    ...
    students = mergeArrays(students, cloudData.students || []);
    hoursLog = mergeArrays(hoursLog, cloudData.hoursLog || []);
    marks = mergeArrays(marks, cloudData.marks || []);
    attendance = mergeArrays(attendance, cloudData.attendance || []);
    payments = mergeArrays(payments, cloudData.payments || []);
    paymentActivity = mergeArrays(paymentActivity, cloudData.paymentActivity || []);
    
    // Merge field memory (cloud wins)
    if (cloudData.fieldMemory) {
        fieldMemory = { ...fieldMemory, ...cloudData.fieldMemory };
        changesMade = true;
    }
    ...
    if (typeof Auth !== 'undefined' && Auth.isAuthenticated()) {
        ...
        if (user) {
            if (cloudSync.syncing) {
                ...
                // Show sync indicator if there are pending changes
                if (hasLocalChanges()) {
        ...
        <div class="modal-content" style="max-width: 500px;">
            ...
            <div class="modal-body">
                <div class="sync-status-info">
                    ...
                    <p><strong>Cloud ID:</strong> <span id="menuBinId">Not created</span></p>
                ...
                <div class="sync-actions" style="margin: 20px 0; display: flex; gap: 10px; flex-wrap: wrap;">
                    <button onclick="manualSyncToCloud()" class="btn" id="menuSyncBtn">
                        🔄 Sync to Cloud Now
                    </button>
                    <button onclick="loadFromCloud()" class="btn">
                        📥 Load from Cloud
                    </button>
                ...
                <div class="sync-info" style="padding: 10px; background: #e7f3ff; border-radius: 4px; font-size: 12px; color: #0066cc;">
                    <strong>🔒 Secure & Private:</strong> Your data is encrypted and only accessible with your unique ID. 
                    We use JSONBin.io for secure cloud storage.
                </div>
            </div>
        </div>
    `;
...
function updateCloudSyncMenu() {
    ...
    // Update bin ID
    if (menuBinId) {
        menuBinId.textContent = cloudSync.binId 
            ? cloudSync.binId.substring(0, 8) + '...'
            : 'Not created';
    }
    ...
    if (menuDataSize) {
        const dataSize = JSON.stringify({
            students, hoursLog, marks, attendance, payments, paymentActivity, fieldMemory
        }).length;
        menuDataSize.textContent = (dataSize / 1024).toFixed(1) + ' KB';
    }
    
    // Update statistics
    if (menuStudentsCount) menuStudentsCount.textContent = students.length;
    if (menuHoursCount) menuHoursCount.textContent = hoursLog.length;
    if (menuMarksCount) menuMarksCount.textContent = marks.length;
    if (menuPaymentsCount) menuPaymentsCount.textContent = payments.length;
}
...
function showCloudSyncInfo() {
    alert(`🌐 How Cloud Sync Works:

✅ Automatic: Your data syncs automatically every 5 minutes
✅ Secure: Data is encrypted and private to your account
✅ Cross-Device: Access your data from any device
✅ Offline: Works without internet, syncs when connected
✅ Free: Uses JSONBin.io free tier (up to 500 requests/day)

Your data is stored securely in the cloud and automatically synchronized across all your devices.`);
}

// Update the sync button in main UI to show cloud menu
function setupCloudSyncEventListeners() {
    // Replace the existing sync button behavior
    const syncBtn = document.getElementById('syncBtn');
    if (syncBtn) {
        syncBtn.onclick = showCloudSyncMenu;
    }
}

// Add this CSS for the new sync styles
const cloudSyncStyles = `
.sync-pending {
...
styleSheet.textContent = cloudSyncStyles;
document.head.appendChild(styleSheet);

// Initialize cloud sync when auth is ready
if (typeof Auth !== 'undefined') {
    // Wait for auth to be initialized, then setup cloud sync
    setTimeout(() => {
        initializeCloudSync();
